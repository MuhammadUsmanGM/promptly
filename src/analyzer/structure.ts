...
// Token guardrail. On a big monorepo (10k+ files) the old 500-file list blew up
// context unnecessarily since findRelevantFiles only ever surfaces 8 files to
// the agent. MAX_FILES is enough room for findRelevantFiles to do keyword
// matching across a representative slice of the repo — files are prioritized
// so shallow / keyDir paths are picked first, which is where almost all
// matches land.
const MAX_FILES = 200;
...
export async function detectStructure(
  ...
  async function walk(dir: string, depth: number) {
    ...
    for (const d of subDirs) {
      const relPath = relative(projectPath, join(dir, d.name)).replace(/\\/g, "/");
